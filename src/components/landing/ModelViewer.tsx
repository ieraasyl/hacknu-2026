...
function ModelContent({
  ...
  }, [gl, camera, enableManualRotation, enableManualZoom, minZoom, maxZoom]);
...
// Format-specific loaders — each calls its hook unconditionally at the top level
interface LoaderProps extends Omit<ModelContentProps, 'content'> {
  url: string;
}

function GLBModelInner({ url, ...rest }: LoaderProps) {
  const { scene } = useGLTF(url);
  const content = useMemo(() => scene.clone(), [scene]);
  return <ModelContent content={content} {...rest} />;
}

function FBXModelInner({ url, ...rest }: LoaderProps) {
  const model = useFBX(url);
  const content = useMemo(() => model.clone(), [model]);
  return <ModelContent content={content} {...rest} />;
}

function OBJModelInner({ url, ...rest }: LoaderProps) {
  const model = useLoader(OBJLoader, url);
  const content = useMemo(() => model.clone(), [model]);
  return <ModelContent content={content} {...rest} />;
}

function ModelInner({ url, ...rest }: LoaderProps) {
  const ext = url.split('.').pop()!.toLowerCase();
  if (ext === 'fbx') return <FBXModelInner url={url} {...rest} />;
  if (ext === 'obj') return <OBJModelInner url={url} {...rest} />;
  return <GLBModelInner url={url} {...rest} />;
}
